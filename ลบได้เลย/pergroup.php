...
<?php
	...
	$dbcp = _PERGRO; //dbcp = database connect page ดาต้าเบส ที่ใช้ ในหน้านี้

	$headtext = 'ระบบ PermissionGroup'; //ส่วนหัว Page
	//Setting Page
	//$number_per_page = 10; //กำหนดให้แสดง 10 แถวต่อเพจ

	//start------------------จัดการเรื่องตารางหน้าแสดงผล-----------------------
	$tableallsize = '90%'; //ขนาดตารางทั้งหมด
	$headtable = array('Permission Name Group','Remark', 'การกระทำ'); //หัวตาราง ที่จะนำไป loop หัวตาราง แสดงผล  หน้าแรก
	$counts = count($headtable);  //นับจำนวน สมาชิค ใน Array
	$tablesizes = array('25%','40%', '37%'); //ขาดในแต่ละช่อย  อ่างอิงจาก $headtable
...
<div>
<?php
	echo fperurl($_GET['f'], 'add', '', 'เพิ่ม Group', $wurl3->mp_aper);
?>  
</div>
...
<?php
			...
			echo fperurl($_GET['f'], 'del', "$showid", 'ลบ', $wurl3->mp_dper).fperurl('pergro', '', $showid, 'สิทธิ', '');
?>
                                    </div></td>
                            </tr>
<?php
...
?>	
                        <tr>
                            <td colspan="4" class="center">ไม่พบรายการสมาชิก</td>  
                        </tr>
<?php } ?>
                ...
                <div class="center">
                    <div align="center" id="showdata">
                        <table width="500" border="1" cellspacing="0" cellpadding="0">
                            <tr> 
                                <td colspan="2"><div align="center"><a name="link1">คำนำหน้าชื่อ</a></div></td>
                            </tr>
                            <tr>
                                <td width="120"><div align="right">รหัสคำนำหน้าชื่อ</div></td>
                                <td width="374"><div align="left"><?php echo $mid; ?> </div></td>
                            </tr>
                            <tr>
                                <td><div align="right">ชื่อย่อ Group</div></td>
...
<?php
		...
		$last_id = mysql_result(mysql_query("SELECT Max(substr(g_id,-1))+1 as MaxID from groups"),0,"MaxID"); 
		/* $last_id ดึงข้อมูลสุดท้าย(Max())ของช่อง ID ออกมาจะได้ g1 แล้วใช้ 
		 * Funtion Substr ตัดตัวอักษร ด้านหน้า 1 ตัว จะได้ 1 แล้วนำไป +1 จะได้ 2
		 */
		$group_id='g'.$last_id;
?>
            <div align="center">
                    ...
                    <table width="500" border="0" cellspacing="5" cellpadding="5">
                        <tr>
                            <td colspan="2"><div align="center"><a name="link1">กรอกข้อมูล Group</a></div></td>
                        </tr>
                        <tr>
                            <td width="120"><div align="right">Permission Name Group</div></td>
                            <input name="<?php echo $value[1];?>" type="hidden" id="<?php echo $value[1];?>" value="<?php echo $group_id;?>" size="50">
                            <td width="374"><input name="<?php echo $value[2];?>" type="text" id="<?php echo $value[2];?>" size="50"><?php echo $value[2];?></td>
                        </tr>
                        <tr>
                            <td width="120"><div align="right">หมายเหตุ</div></td>
                            <td width="374"><input name="<?php echo $value[3];?>" type="text" id="<?php echo $value[3];?>" size="50"><?php echo $value[3];?></td>
                        </tr>
                        <tr>
                ...
                <div align="center">
                       <form name="form3" method="post" action="index.php?f=<?php echo $_GET[f];?>&action=saveedit&id=<?php echo $id; ?>"> 
                        <table width="500" border="0" cellspacing="5" cellpadding="5">
                            ...
                            <tr>
                                <td width="120"><div align="right">ชื่อเต็ม Group</div></td>

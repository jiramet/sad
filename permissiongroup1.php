...
<?php
	$s = trim($_POST[s]);
	$Query = $db->query("SELECT * FROM "._WURL." ORDER BY w_id");
	$sum = $db->rows($Query);
	if ($sum) {
		while ($url = $db->fetch($Query)) {
			$m = $db->fetch($db->query("SELECT * FROM "._MPER." WHERE mp_mid = '".$idper."' and mp_wid = '".$url->w_id."' "));
?>
<div align="center">
 <form name="form2" method="post" action="index.php?f=<?php echo $_GET[f];?>&action=saveadd&id=<?php echo $idper;?>&id_menu=<?php echo $url->w_id;?>">
        <tr>
...
<?php
	if ($action == "saveadd") {
		...
		$num = $db->num_rows(_MPER, "mp_mid,mp_wid", "mp_mid='$idper' and mp_wid=".trim($_GET[id_menu]));
		...
		if ($num == 1) { //  ถ้ามีข้อมูลใน Data bases  จะใช้วิธีการ  Update data
			if (fnc0(all) == 1) {
				if ($ch1 == $ch2 and fnc0(all) == 1) {
					$save = $db->update_db(_MPER, array("mp_wper" => 1, "mp_oper" => 1, "mp_aper" => 1, "mp_eper" => 1, "mp_dper" => 1, "mp_sper" => 1), "mp_mid='$idper' and mp_wid=".trim($_GET[id_menu]));
					_go("index.php?f=$_GET[f]&id=$idper");
				} elseif (fnc0(wper) == 0 and $ch2 != 6 and fnc0(all) == 1) {
					$save = $db->update_db(_MPER, array("mp_wper" => 0, "mp_oper" => 0, "mp_aper" => 0, "mp_eper" => 0, "mp_dper" => 0, "mp_sper" => 0), "mp_mid='$idper' and mp_wid=".trim($_GET[id_menu]));
					_go("index.php?f=$_GET[f]&id=$idper");
				} elseif ($ch1 != $ch2) {
					$save = $db->update_db(_MPER, array("mp_wper" => fnc0(wper), "mp_oper" => fnc0(oper), "mp_aper" => fnc0(aper), "mp_eper" => fnc0(eper), "mp_dper" => fnc0(dper), "mp_sper" => fnc0(sper)), "mp_mid='$idper' and mp_wid=".trim($_GET[id_menu]));
					_go("index.php?f=$_GET[f]&id=$idper");
				}
			...
			getJavaAlert("กรุณา".trim($_GET[id_menu]), 0);
			_go("index.php?f=$_GET[f]&id=$idper");
		} elseif (fnc0(wper) == 0) {
